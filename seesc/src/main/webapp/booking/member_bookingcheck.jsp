...
int user_idx = request.getParameter("user_idx")==null||request.getParameter("user_idx").equals("")?-1:Integer.parseInt(request.getParameter("user_idx"));


ArrayList<BookingDTO> arr = boodao.boomanage(user_idx);
		...
		<article>
			...
			<table>
				...
				<%for(int i=0;i<arr.size();i++){
					...
					<td><input type="button" value="예약 취소" onclick ="javascript: var result =window.confirm('에약을 취소하시겠습니까?');if(result){location.href ='bookingCancle.jsp?booking_idx=<%=arr.get(i).getBooking_idx()%>'}">
					<%
				} %>
