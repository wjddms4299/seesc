	...
	%>
	 ...
	 if(user_idx!=null){
		 int manager=udao.mngnum(user_idx);
	 	 if(manager==1){%>
	 		 <a href="/seesc/booking/bookingStep01_up.jsp?thema_idx=<%=idx_idx.get(j)%>"><input type="button" name="themaUpdate" value="테마 수정"></a> &nbsp; <input type="button" name="themaDelete" value="테마 삭제" onclick="window.open('/seesc/booking/del_popup.jsp?thema_idx=<%=idx_idx.get(j)%>','del_popup.jsp','width=550,height=300,top=100,left=300');">
	 	 <%}%>
	 <%}%></div>
	 <hr width="950">
	</article>
	<article id="a2">
	 <img alt="방탈출 <%=idx_idx.get(j)%>" src="/seesc/thema_img/00<%=idx_idx.get(j)%>.jpg" width="300" height="400" class="a2-0">
	</article>
	<article id="a3">
	 <br><br><br><br>
	 ...
	 %>
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_idx.get(j)%>&time_date=<%=time_date%>&time_ptime=1"><input type="button" <%=booking_idx.get(j*6)>0?"value='10:00 예약마감' disabled":"value='10:00 예약가능'"%> class="a2-1"></a> &nbsp;
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_idx.get(j)%>&time_date=<%=time_date%>&time_ptime=2"><input type="button" <%=booking_idx.get(j*6+1)>0?"value='12:00 예약마감' disabled":"value='12:00 예약가능'"%>></a> &nbsp;
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_idx.get(j)%>&time_date=<%=time_date%>&time_ptime=3"><input type="button" <%=booking_idx.get(j*6+2)>0?"value='14:00 예약마감' disabled":"value='14:00 예약가능'"%>></a><br>
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_idx.get(j)%>&time_date=<%=time_date%>&time_ptime=4"><input type="button" <%=booking_idx.get(j*6+3)>0?"value='16:00 예약마감' disabled":"value='16:00 예약가능'"%> class="a2-1"></a> &nbsp;
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_idx.get(j)%>&time_date=<%=time_date%>&time_ptime=5"><input type="button" <%=booking_idx.get(j*6+4)>0?"value='18:00 예약마감' disabled":"value='18:00 예약가능'"%>></a> &nbsp;
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_idx.get(j)%>&time_date=<%=time_date%>&time_ptime=6"><input type="button" <%=booking_idx.get(j*6+5)>0?"value='20:00 예약마감' disabled":"value='20:00 예약가능'"%>></a><br><br><br><br><br><br><br><br><br>
	</article>
<%}%>
<article>
 <%
