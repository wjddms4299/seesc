...
<script>
function popup_open(){
	window.open('/seesc/booking/cancle_popup.jsp?booking_idx=<%=booking_idx%>&booking_pay_ok=<%=booking_pay_ok%>&thema_idx=<%=thema_idx%>&time_date=<%=time_date%>&time_ptime=<%=time_ptime%>','cancle_popup.jsp','width=600,height=450,top=100,left=300');
}
</script>
</head>
<body>
