 ...
 <table id="a1-1" cellspacing="10">
 	...
 	for(int z=0;z<10;z++){
		...
		w_i[z]=now.get(Calendar.DAY_OF_WEEK);
 		...
 		if(w_i[z]==1){
 			%>
 			<td id="a1-2"><a href="bookingStep01.jsp?time_date=<%=ymd[z]%>" style="color:red;"><h3><%out.print(d[z]);%></h3></a></td>
 			<%
 		}else if(w_i[z]==7){
 			%>
 			<td id="a1-3"><a href="bookingStep01.jsp?time_date=<%=ymd[z]%>" style="color:#327FFA;"><h3><%out.print(d[z]);%></h3></a></td>
 			<%
 		}else{
 			%>
 			<td><a href="bookingStep01.jsp?time_date=<%=ymd[z]%>"><h3><%out.print(d[z]);%></h3></a></td>
 			<%
 		}
	...
	%>
	 ...
	 if(user_idx!=null){
		 int manager=udao.mngnum(user_idx);
	 	 if(manager==1){%>
	 		 <a href="/seesc/booking/bookingStep01_up.jsp?thema_idx=<%=idx_level.get(j)%>"><input type="button" name="themaUpdate" value="테마 수정" style="height:25px;width:80px;"></a> &nbsp; <input type="button" name="themaDelete" value="테마 삭제" onclick="window.open('/seesc/booking/del_popup.jsp?thema_idx=<%=idx_level.get(j)%>','del_popup.jsp','width=550,height=300,top=100,left=300');" style="height:25px;width:80px;">
	 	 <%}%>
	 <%}%></div>
	 <hr width="950">
	</article>
	<article id="a2">
	 <img alt="방탈출 <%=idx_level.get(j)%>" src="/seesc/thema_img/<%=thdao.imgSelect_N(dto.get(j).getImg_idx())%>" width="300" height="400" class="a2-0">
	</article>
	<article id="a3">
	 <br><br><br><br>
	 ...
	 %>
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_level.get(j)%>&time_date=<%=time_date%>&time_ptime=1"><input type="button" <%
	 ...
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_level.get(j)%>&time_date=<%=time_date%>&time_ptime=2"><input type="button" <%
	 ...
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_level.get(j)%>&time_date=<%=time_date%>&time_ptime=3"><input type="button" <%
	 ...
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_level.get(j)%>&time_date=<%=time_date%>&time_ptime=4"><input type="button" <%
	 ...
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_level.get(j)%>&time_date=<%=time_date%>&time_ptime=5"><input type="button" <%
	 ...
	 <a href="bookingStep02.jsp?thema_idx=<%=idx_level.get(j)%>&time_date=<%=time_date%>&time_ptime=6"><input type="button" <%
