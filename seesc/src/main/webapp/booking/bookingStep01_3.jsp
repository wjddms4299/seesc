	...
	%>
	 ...
	 if(user_idx!=null){
		 int manager=udao.mngnum(user_idx);
	 	 if(manager==1){%>
	 		 <a href="/seesc/booking/bookingStep01_up.jsp?thema_idx=<%=idx_booking.get(j)%>"><input type="button" name="themaUpdate" value="테마 수정"></a> &nbsp; <input type="button" name="themaDelete" value="테마 삭제" onclick="window.open('/seesc/booking/del_popup.jsp?thema_idx=<%=idx_booking.get(j)%>','del_popup.jsp','width=550,height=300,top=100,left=300');">
	 	 <%}%>
	 <%}%></div>
	 <hr width="950">
	</article>
